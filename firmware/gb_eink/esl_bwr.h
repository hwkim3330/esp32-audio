...
//
// keep_open=true 면 **연결을 끊지 않고 남긴다.** 같은 태그에 연달아 보낼 때 그 다음
// 호출이 연결 수립과 대기를 통째로 건너뛴다. 원래는 전송마다 disconnect(600ms) +
// connect + delay(300ms) + 끝에 disconnect(600ms) 를 해서, 실측 4074ms 중 1.5초쯤이
// 링크를 다시 세우는 데 쓰였다. 게임처럼 같은 한 장을 계속 고쳐 쓰는 용도에서는
// 링크를 붙들고 있는 것이 맞다.
//
// 주의: 링크를 붙들면 그 시간 동안 BLE 중앙 자원을 쓴다. 센싱 스캔과 겸용하는
// 스케치(radar_display)에서는 기본값(false)을 유지할 것.
EslResult esl_upload(const uint8_t addr[6], const uint8_t *payload, size_t len,
                     uint32_t *ms_out, uint32_t *parts_out, bool keep_open = false);

// keep_open 으로 남겨둔 연결을 닫는다. 스캔을 하거나 다른 태그로 옮길 때 부른다.
void esl_disconnect(void);
