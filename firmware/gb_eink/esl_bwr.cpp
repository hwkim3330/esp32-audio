...
static uint8_t s_cur_addr[6] = {0};
static bool    s_cur_valid = false;

void esl_disconnect(void)
{
    if (s_cli && s_cli->isConnected()) { s_cli->disconnect(); delay(600); }
    s_cur_valid = false;
}
                     ...
                     uint32_t *ms_out, uint32_t *parts_out, bool keep_open)
{
    ...
    const bool reuse = cli->isConnected() && s_cur_valid &&
                       !memcmp(s_cur_addr, addr, 6);
    ...
    // keep_open 이면 링크를 남긴다 — 다음 전송이 연결 수립을 건너뛴다.
    // 실패했을 때는 상태가 의심스러우므로 항상 끊고 다음에 새로 맺는다.
    if (!keep_open || res != ESL_OK) {
