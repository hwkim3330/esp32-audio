...
//
// 대신 데이터가 실제로 지지하는 것을 그린다. 전자종이를 안 쓰면 웹이 네 페이지 몫을
// 전부 받아야 하므로, 지금은 여기에 다 있다:
//   1. CSI 워터폴 — 서브캐리어 64개 × 시간. **편차 z** 를 그린다(원시 진폭이 아니다).
//      눈에 보이는 것과 점수의 출처가 같아야 한다.
//   2. 방 도면 — 보드와 앵커 4개, 각 링크의 편차(z)를 굵기·색으로. **진짜 공간 정보다.**
//      논문들은 안테나가 한 곳에 모여 있지만 우리 앵커는 방 네 지점에 퍼져 있다.
//      단 방위는 모르므로 원주에 균등 배치한 **도식**이다 — 위치를 주장하지 않는다.
//   3. 추세 4개 시간 축(72s/12min/1hr/6hr) 전부. 종이에는 6시간만 그렸다.
//   4. 사건 목록 · 24시간 히스토그램 · 검증 판정(Cohen's d).
//
// 차트는 uPlot(MIT)이다. gzip 해서 플래시에 넣는다(tools/embed_web.py) — SoftAP 에는
// 인터넷이 없어서 CDN 을 쓸 수 없다. 22.7KB 면 축·눈금·범례·툴팁을 손으로 그리는 것보다
// 싸다(예전 판은 손으로 그려서 축이 아예 없었다).
//
// 검증 전에는 단정하지 않는다 — 전자종이와 같은 규칙이다. `verified` 가 0 이면 큰 글씨가
// MOTION 이 아니라 BAND HIGH/LOW 이고 NOT HUMAN-VERIFIED 배지가 붙는다.
//
